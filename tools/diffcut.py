...
class DiffCut:
    ...
    def get_affinity_matrix(self, affinity, level):
        ...
        null_values_idx = set(
            torch.unique(torch.where(affinity == 0)[0]).tolist()
        )
    ...
    def get_bipartition(self, y, A, D, k=100):
        ...
        partitions = [1.0 * (y > thresh) for thresh in thresholds]
        N_cut_values = []
        L = D - A
        sum_D = torch.sum(D)
        diag_D = torch.diag(D)
    ...
    def get_masked_affinity_matrix(self, painting, affinity, mask):
        ...
        mask_affinity = torch.t(painting.reshape(1, -1)) @ painting.reshape(
            1, -1
        )
    ...
    def recursive_ncut(
        ...
        affinity, painting = self.get_masked_affinity_matrix(
            painting, affinity, mask
        )
        ...
        if A.shape[0] > 1:
            ...
            second_smallest_vec = self.deterministic_vector_sign_flip(
                second_smallest_vec
            )
            ...
            bipartition = 1.0 * null_vec.reshape(dims)

            if ncut < tau and self.counter < 16:
    ...
    def assemble_clusters(self, clusters, h, w):
        """
        return final_mask, count_classes
        """
    ...
    def generate_masks(
        ...
        features,
        tau=0.5,
        patch_size=16,  # mask_size=(128, 128),
        # masks=None,
        alpha=10,
        # img_size=1024,
        num_patches_h=32,
        ...
        feats = (
            ...
            )  # теперь это (1, 64, 64, C)
            .permute(0, 3, 1, 2)  # стало (1, C, 64, 64)
            .to(torch.float32)
        )

        # feats = (
        #     features.reshape(1, img_size // patch_size, img_size // patch_size, -1)
        #     .permute(0, 3, 1, 2)
        #     .to(torch.float32)
        # )
        ...
        # Assemble clusters
        masks = self.assemble_clusters(clusters, h, w)
        masks = masks
