...
def dense_crf(img, probs):
    ...
    # 2. Инициализируем CRF
    d = dcrf.DenseCRF2D(w, h, c)
    
    # 3. Подготавливаем Unary Energy
    # Вход должен быть (C, H*W). float32 — критически важно.
    unary = unary_from_softmax(probs.astype(np.float32))
    d.setUnaryEnergy(unary)

    # 4. Добавляем потенциалы без использования проблемных NORM_ALIZE
    # По умолчанию библиотека сама использует нужную нормализацию.
    
    # Gaussian: отвечает за гладкость (чтобы не было "дырок" в маске)
    d.addPairwiseGaussian(sxy=(3, 3), compat=3)

    # Bilateral: отвечает за привязку к краям и цветам на фото
    d.addPairwiseBilateral(sxy=(80, 80), srgb=(13, 13, 13), rgbim=img, compat=10)

    # 5. Запускаем инференс
    q = d.inference(10)
    
    # 6. Возвращаем результат в исходной форме (C, H, W)
...
def visualize_segmentation(
    ...
    plt.subplot(1, 2, 2)
    plt.imshow(orig_img)
    plt.imshow(pred_mask, alpha=0.8, cmap="tab20")
...
class CocoClaspDataset(Dataset):
    ...
    def __getitem__(self, index):
        ...
        f_name = img_info.get("file_name") or os.path.basename(img_info.get("coco_url", ""))
        
        if not f_name:
            raise KeyError(f"Не нашел ни file_name, ни coco_url в img_info: {img_info}")
...
if __name__ == "__main__":
    ...
    for i in range(n):
        ...
        mask_one_hot = np.zeros((best_k, num_patches_h, num_patches_w), dtype=np.float32)
        for k in range(best_k):
            mask_one_hot[k] = (mask_small == k).astype(np.float32)
        mask_tensor = torch.from_numpy(mask_one_hot).unsqueeze(0)
        full_mask_probs = F.interpolate(
            mask_tensor, 
            size=(orig_h, orig_w), 
            mode="bilinear", 
            align_corners=False
        ...
        refined_probs = dense_crf(original_image, full_mask_probs)
